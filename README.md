...
- 每次 commit 视作一次修订，需递增修订号
- 版本号后可附加 `alpha`、`beta` ，分别用于指示内部测试版（内测版）和公共测试版（公测版）
- 主版本为 0 的版本必须添加 `alpha` 后缀
- 正式发布版版本不带后缀

例如，`0.1.11-alpha`、`1.0.1`、`1.0.2-beta` 是正确的版本，而 `0.1.1`、`0.1.11-beta` 是错误的版本。
...
| notes                    | TEXT | 习惯的备注信息                                                        |
| supervisionMethod        | TEXT | 监督方式（LOCAL_NOTIFICATION_ONLY、EMAIL_REPORTING 或 TEXT_REPORTING） |
| supervisorEmailAddresses | TEXT | 监督人邮箱地址列表（以 JSON 格式存储）                                         |
| supervisorPhoneNumbers   | TEXT | 监督人手机号列表（以 JSON 格式存储）                                          |
