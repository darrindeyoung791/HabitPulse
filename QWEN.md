...
- ✅ **空态居中修正 + 记录搜索 + Omnibox AI 快捷 chip + AI 对话清理与顶栏下拉收回重建** - 版本 0.8.62-alpha (210) 五项体验修复与增强：
  ...
  - **顶栏反向拖拽收回重建**：原第十一次迭代实现的该功能在后续重构中丢失（AIChatScreen 无任何 pointerInput 残留），按原方案重写——TopAppBar 容器挂自定义纵向检测器（同主页把手模式：守卫一律在 awaitFirstDown 之后、越过 slop 且向下位移率先越阈才 engage、engage 前零消费不影响顶栏按钮点击；仅嵌入模式且 AI 未输出时启用，独立 Activity 不挂载），跟手 snapTo 反向擦洗 progress（行程经新增 `collapseTravelPx: () -> Float` 参数由 AiSheetOverlay 提供「把手到屏幕顶部实测距离」，与底部上滑手势速率对称）；松手动量投影 `DrawerFlingProjectionSeconds(0.16s)` 取最近锚点：<0.5 走 onCollapse 标准收起（主页侧完成草稿归还/触感），≥0.5 本地 spring(dampingRatio 0.8 / stiffness 280) + 释放初速（钳 ±8）弹回展开态并在落定后震动一次
